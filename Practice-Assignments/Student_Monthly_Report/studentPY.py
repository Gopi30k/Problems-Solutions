...
def printOut(dated):
	...
	cursor = db.cursor()
	#query="""select s.sId,s.sName,s.sClassNo,m.English,m.Tamil,m.Maths,m.Science,m.Social from Student s join Marks m on m.sId=s.sId where examDate like %s"""
...
from Student s join Marks m on m.sId=s.sId where examDate like %s;"""
	cursor.execute(query,(dated,))
...
put=[]
for i in monthList:
	for j in monthdict[i]:
		put.append( printOut(j))

for i in put:
	createHTMLtable(i,'JAN')
